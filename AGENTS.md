...
- **Mobile card tactility**: `CombatView` keeps dragged cards above the touch point, expands touch target hit-testing, shows a persistent release guide, and labels targetable enemies with the exact card about to be played. Haptics go only through the guarded `haptic()` helper (`game.isTouch()` + `navigator.vibrate`) and should mark state edges (lift, valid target, commit, rejection), not continuous pointer movement. Preserve `lostpointercapture` cleanup and call `hideDragGuide()` on every drag exit path.

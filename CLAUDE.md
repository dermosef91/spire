...
- `combat/combat.js` — the turn-based engine (energy, piles, powers, orbs,
  ...
  **Smarter-enemy hooks (all data-driven, one-shot):**
  ...
  (5) **Blueprint death hooks** — `bp.onDeath(c, s)` on the dying enemy and `bp.onAllyDeath(c, s, dead)` on every living ally, fired from `onEnemyDeath` (never on a flee — `enemyFlee` doesn't route through it). Used by `market_thief` (killed before fleeing → refunds everything it stole, tracked in `s._stolen`, + a 15 bounty; its steals escalate 8/12/16 across `swipe`/`swipe2`/`swipe3` and the escape is telegraphed with the `flee` intent type) and `husk_drone` (a survivor salvages a dead *sibling drone*: sets `_scavenge` + re-picks its intent to the telegraphed Salvage Core — the tutorial's solo drone can never trigger it, so its guaranteed turn-1 `zap` stays safe).
  **Act 1 context-aware AI pass (2026-07)**: Act 1 predated the smarter-AI helpers; now `static_jackal` lunges at an unblocked player and howls at a blocked one (pack howl dedup by reading a living packmate's `e.move` — enemies pick intents sequentially, so the second jackal sees the first's choice), `spark_imp` telegraphs Burn Out (a self-destruct nuke) once kindled to 6+ Resolve, `tide_priest`'s Baptize strips all debuffs from the most-afflicted ally when someone carries 4+ total stacks (via `debuffStacks()`, which reads `POWERS[key].type === 'debuff'` — enemies.js now imports keywords.js for this), and `reef_spitter` shells up below 40% HP (never twice running).
...
- **Gold-stealing enemy moves must telegraph and confirm the theft** (established
  ...
  **Adding a brand-new intent *type* touches five places** (done for
  `counter` and `flee`): `INTENT` + `INTENT_INFO` (`icons.js`), a
  `renderIntent` branch (`combatView.js`), **both** `.intent-*` selector
  lists in `game.js`, and the `.intent-*` color + cursor rule lists in
  `styles.css` — miss the game.js/styles ones and the icon renders but its
  click-popup silently doesn't open.

// Potion art loader — checks if a custom generated PNG potion art is available,
// falling back to the inline SVG models. The manifest is loaded once on first use
// from assets/potion-art/manifest.json.
//
...
function ensureManifest() {
  ...
  return fetch('assets/potion-art/manifest.json')
    ...
    .catch(() => {
      // No manifest = no potion art; every call falls back to SVG
      _manifest = { ids: [] };
      _loaded = true;
      _waiters.forEach(fn => fn(_manifest));
      _waiters.length = 0;
...
/**
 * Returns true if a custom potion art PNG exists for the given potion id.
 * SYNCHRONOUS — returns false until the manifest is loaded.
 */
export function hasPotionArt(id) {

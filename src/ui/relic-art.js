// Relic art loader — checks if a custom generated PNG relic art is available,
// falling back to the inline SVG models. The manifest is loaded once on first use
// from assets/relic-art/manifest.json.
//
...
function ensureManifest() {
  ...
  return fetch('assets/relic-art/manifest.json')
    ...
    .catch(() => {
      // No manifest = no relic art; every call falls back to SVG
      _manifest = { ids: [] };
      _loaded = true;
      _waiters.forEach(fn => fn(_manifest));
      _waiters.length = 0;
...
/**
 * Returns true if a custom relic art PNG exists for the given relic id.
 * SYNCHRONOUS — returns false until the manifest is loaded.
 */
export function hasRelicArt(id) {

#!/usr/bin/env node
// gen-potion-art.js — Dev-only potion art generator for ÀṢẸ: Ascend the Obsidian Spire
// Reads tools/potions.manifest.json, generates consistent on-theme PNG potion artworks
// via the OpenAI image API, post-processes with sharp, writes to assets/potion-art/.
//
// Usage:
//   node tools/gen-potion-art.js                  # generate missing potion art
//   node tools/gen-potion-art.js --force           # regenerate all
//   node tools/gen-potion-art.js --dry-run         # placeholder PNGs, no API calls
//   node tools/gen-potion-art.js --ids elixir      # only specific ids
//   OPENAI_API_KEY=sk-... node tools/gen-potion-art.js
//
...
const POTION_ART_DIR = join(ROOT, 'assets', 'potion-art');
const RELIC_ART_DIR = join(ROOT, 'assets', 'relic-art');
const MANIFEST_IN = join(__dirname, 'potions.manifest.json');
const MANIFEST_OUT = join(POTION_ART_DIR, 'manifest.json');
...
const STYLE_BIBLE = `Afrofuturist dark graphic illustration of a single potion flask vessel in a bold woodcut / risograph screen-print style. Bold black ink linework, very high contrast, strictly limited palette of ember orange #ff6a1a, deep ember #e8431a, amber #ffab47, cream #f3e8d8 — absolutely no other hues. Focus on a clean, distinct silhouette of a flask/vessel with simple bold shapes, optimized for readability at a very small icon scale. Geometric African-inspired ornamental patterns on the vessel. No glowing halos, no concentric floating circle sigils, no background star fields, no ground shadow, no text, no border, no UI chrome. Transparent background.`;
...
function logCost(id, endpoint, model) {
  const cost = 0.04; // Approximate costs (gpt-image-2 1024x1024)
  totalCost += cost;
...
async function createPlaceholderWithSharp(sharp, id, rarity) {
  ...
  const accentColor = '#e8431a';
  const svg = `<svg width="${size}" height="${size}" xmlns="http://www.w3.org/2000/svg">
    <path d="M${size*0.42} ${size*0.22} H${size*0.58} V${size*0.36} L${size*0.68} ${size*0.64} C${size*0.72} ${size*0.76} ${size*0.62} ${size*0.84} ${size*0.50} ${size*0.84} C${size*0.38} ${size*0.84} ${size*0.28} ${size*0.76} ${size*0.32} ${size*0.64} L${size*0.42} ${size*0.36}Z" fill="${bgColor}" stroke="${accentColor}" stroke-width="12"/>
    <text x="${size/2}" y="${size/2 - 20}" text-anchor="middle" fill="#ffab47" font-size="28" font-family="monospace" font-weight="bold">${id}</text>
    <text x="${size/2}" y="${size/2 + 20}" text-anchor="middle" fill="#f3e8d8" font-size="18" font-family="monospace">${rarity || 'potion'}</text>
...
async function generatePotion(openai, OpenAI, sharp, potion) {
  const { id, prompt } = potion;
  const outPath = join(POTION_ART_DIR, `${id}.png`);
  ...
  if (DRY_RUN) {
    const buf = await createPlaceholderWithSharp(sharp, id, 'potion');
  ...
  // Read style-key from relic-art as image input for consistency
...
async function main() {
  console.log('╔══════════════════════════════════════════════════════════╗');
  console.log('║  ÀṢẸ — Potion Art Generator                              ║');
  console.log('╚══════════════════════════════════════════════════════════╝');
  ...
  let potions = manifest;
  if (ID_FILTER) {
    potions = potions.filter(e => ID_FILTER.includes(e.id));
  }

  console.log(`  Found ${potions.length} potions to process`);
  console.log();

  mkdirSync(POTION_ART_DIR, { recursive: true });
  ...
  // Check style-key is available (reusing relic one)
  if (!existsSync(STYLE_KEY_PATH)) {
    console.error(`❌ Style calibration reference image not found at ${STYLE_KEY_PATH}`);
    console.error('   Please run node tools/gen-relic-art.js first to generate it.');
    process.exit(1);
  }
  ...
  // Generate each potion
  console.log('━━━ Generating potions ━━━');
  const generated = [];
  for (const potion of potions) {
    try {
      const id = await generatePotion(openai, OpenAIClass, sharp, potion);
      if (id) generated.push(id);
    } catch (err) {
      console.error(`  ❌ ${potion.id} failed: ${err.message}`);
    }
  }
  console.log();
  ...
  const allPngs = readdirSync(POTION_ART_DIR)
  ...
  console.log(`📋 Manifest written: ${allPngs.length} potions → assets/potion-art/manifest.json`);
